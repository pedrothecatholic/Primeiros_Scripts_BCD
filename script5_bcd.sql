...
#Retorna a data e a hora atualizado no servidor
select current_timestamp() as dataHoraAtualizado;

#FORMATAÇÃO DE HORA

select time_format(current_time(), '%H:%i:%s') as hora; 
#select time_format(current time(), '%H') as hora
#select time_format(current time(), '%i') as minuto,
#select time_format(current time(), '%s') as segundo,
#select time_format(current time(), '%H:%i') as horaMinuto;
...
select date_format(currten_date(), '%Y') as ano;

select date_format(current_date(), '%d/%m/%Y') as dataBrasil;
select date_format(current_date(), '%Y-%m-%d') as dataniversal;
...
select timestampdiff(DAY, '2021-11-22 15:00:00', '2021-11-23 22:00:00') as diferencaDias;
